...
namespace hex_kurban{
	...
	HexAdepter<T>::HexAdepter():AbstractHex(11){
		Cell a;
	...
	template <template<class ...>class T>
	HexAdepter<T>::HexAdepter(const int& _size):AbstractHex(_size){
		Cell a;
		HexCells.resize(getSize());//WE RESIZE THE VECTOR
		temp_arrey.resize(getSize());
		for (int i = 0; i < getSize(); i++)
	...
	template <template<class ...>class T> 
	void HexAdepter<T>::readFromFile(string file_name1){
	...
	template <template<class ...>class T> 
	Cell& HexAdepter<T>::gettempCell(int i,int j){
		return temp_arrey[i][j];
	}
	template <template<class ...>class T> 
	Cell& HexAdepter<T>::operator()(int i,int j){
		try{
