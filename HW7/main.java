
public class main
{
	public static void main(String[] args){
		//boolean control=false;
		//int lastfullcell=0;
		hexgame h1=new hexgame(11);
		//hexgame h4=new hexgame(8);
		//hexgame h5=new hexgame(14);
		//control=false;
		//HEX h6=11;
		//lastfullcell=0;//LAST FILLED CELL OF ALL GAME
		//h1.playGame();//START GAME
		//h1.findMarked();//FIND HOW MANY   CELL HAS FILLED
		//h1.setfullcell(h1.getCount());//FILLED CELL OF THIS GAME
		//h1.setCount(0);//TO MAKE STATIC MEMBER ZERO FOR THE OTHER GAME STARTING WITH ZERO FILLED CELL TO CALCULATED TRUE 
		/*System.out.println("---------------	NEXT GAME ------------------");
		h2.playGame();
		h2.findMarked();
		h2.setfullcell(h2.getCount());
		h2.setCount(0);
		System.out.println("--------------	NEXT GAME -------------------");
		if(control==false){
			System.out.println("h2 has more marked cell only user ");
		}
		else if(control==true){
			System.out.println("h1 has more marked cell only user ");
		}
		control=false;
		hexgame h3=new hexgame(11);
		h3.playGame();
		h3.findMarked();
		h3.setfullcell(h3.getCount());
		h3.setCount(0);
		System.out.println("---------------- NEXT GAME --------------");
		hexgame h4=new hexgame(14);
		h4.playGame();
		h4.findMarked();
		h4.setfullcell(h4.getCount());
		control=h3==h4;//== OPERATOR 
		h4.setCount(0);
		if(control==true){
			System.out.println("h3 has more marked cell only user ");
		}
		else if(control==false){
			System.out.println("h4 has more marked cell only user ");
		}
		System.out.println("--------------  NEXT GAME ----------------");
		h5.playGame();
		h5.findMarked();
		h5.setfullcell(h5.getCount());
		h5.setCount(0);
		lastfullcell=h1.getfullcell()+h2.getfullcell()+h3.getfullcell()+h4.getfullcell()+h5.getfullcell();//LASTFULLCELL OF ALL GAME 
		System.out.println("The number of marked cell (NOT empty) of all game = "+lastfullcell);*/
	}
}